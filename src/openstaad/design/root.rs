use crate::openstaad::{
    ...
    tools::variant::{SafeArray, variant_from_raw_pointer},
};
...
impl<'a> Design<'a> {
    ...
    pub fn assign_design_command(
        ...
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.dispatch, "AssignDesignCommand", &mut params);
    ...
    pub fn assign_design_group(
        ...
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.dispatch, "AssignDesignGroup", &mut params);
            match result_variant {
                Ok(v) => {
                    let result_code = VariantToInt32(&v as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn create_design_brief(&self, design_code: i32) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.dispatch, "CreateDesignBrief", &mut params);
            match result_variant {
                Ok(v) => {
    ...
    pub fn get_design_brief_code(&self, brief_ref: i32) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.dispatch, "GetDesignBriefCode", &mut params);
            match result_variant {
                Ok(v) => {
