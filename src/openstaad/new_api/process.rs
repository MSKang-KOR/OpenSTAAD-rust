...
use crate::openstaad::{
    new_api::{
        geometry::Geometry,
        root::Root,
        utils::{StaadObject, execute_method},
    },
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        match self.connect_to_staad() {
            Ok(_) => {
                println!("STAAD.exe와 COM 연결 성공!");
                let root = self.root();
                let pid = match execute_method(&StaadObject::Root(root), "GetProcessId", &[]) {
                    Ok(v) => v.as_u64().unwrap(),
                    Err(e) => bail!("Fail to connect for GetProcessId method: {}", e),
                };
