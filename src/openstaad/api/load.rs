...
// Definitions :: Reference Load

// Definitions :: Direct Analysis
...
impl<'a> Load<'a> {
    ...
    pub fn delete_wind_definition(&self, type_no: i32) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant = invoke_method(&self.dispatch, "DeleteWindDefinition", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn add_seismic_definition(
        ...
        unsafe {
            ...
            let result_variant = invoke_method(&self.dispatch, "AddSeismicDefinition", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code == 1)
    ...
    pub fn modify_seismic_definition_params(
        ...
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.dispatch, "ModifySeismicDefinitionParams", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
