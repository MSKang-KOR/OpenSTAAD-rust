...
impl<'a> Section<'a> {
    ...
    pub fn create_upt_table(&self, table_type: i32) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "CreateUPTTable", &mut params);
            match result_variant {
                Ok(var) => {
    ...
    pub fn assign_beta_angle(&self, beam_nos: Vec<i32>, beta_angle: f64) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "AssignBetaAngle", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn get_beta_angle(&self, beam_no: i32) -> Result<f64, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "GetBetaAngle", &mut params);
            match result_variant {
                Ok(var) => {
    ...
    pub fn get_section_property_count(&self) -> Result<i32, anyErr> {
        let result_variant = unsafe {
            invoke_method(&self.property.dispatch, "GetSectionPropertyCount", &mut [])
        };
        match result_variant {
            Ok(var) => {
                let count = unsafe { VariantToInt32(&var as *const VARIANT).unwrap() };
    ...
    pub fn get_shape_code(&self, country: i32, section_name: &str) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "GetShapeCode", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn create_upt_table_ex(&self, table_no: i32, table_type: i32) -> Result<i32, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "CreateUPTTableEx", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn find_upt_table(&self, table_type: i32) -> Result<i32, anyErr> {
        unsafe {
            let mut params = [VARIANT::from(table_type)];
            let result_variant =
                invoke_method(&self.property.dispatch, "FindUPTTable", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code)
    ...
    pub fn remove_upt_table(&self, table_ref: i32) -> Result<bool, anyErr> {
        unsafe {
            ...
            let result_variant =
                invoke_method(&self.property.dispatch, "RemoveUPTTable", &mut params);
            match result_variant {
                Ok(var) => {
                    let result_code = VariantToInt32(&var as *const VARIANT).unwrap();
                    anyOk(result_code == 1)
