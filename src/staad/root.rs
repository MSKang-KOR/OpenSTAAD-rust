...
use crate::staad::{
    ...
    property::root::Property, section::Section, support::root::Support,
...
impl Root {
    ...
    pub fn analyze(&self) -> Result<(), anyErr> {
        unsafe {
            let result_variant =
                invoke_method_with_result(self.dispatch.as_ref().unwrap(), "Analyze", &mut []);
    ...
    pub fn get_error_message(&self) -> Result<String, anyErr> {
        let result_variant = unsafe {
            invoke_method_with_result(self.dispatch.as_ref().unwrap(), "GetErrorMessage", &mut [])
        };
        match result_variant {
            Ok(var) => {
    ...
    pub fn get_process_handle(&self) -> Result<i64, anyErr> {
        let result_variant = unsafe {
            invoke_method_with_result(self.dispatch.as_ref().unwrap(), "GetProcessHandle", &mut [])
        };
        match result_variant {
            Ok(var) => {
                let handle = unsafe { VariantToInt64(&var as *const VARIANT).unwrap() };
    ...
    pub fn get_process_id(&self) -> Result<i32, anyErr> {
        let result_variant = unsafe {
            invoke_method_with_result(self.dispatch.as_ref().unwrap(), "GetProcessId", &mut [])
        };
        match result_variant {
            Ok(var) => {
    ...
    pub fn is_analyzing(&self) -> Result<bool, anyErr> {
        let result_variant = unsafe {
            invoke_method_with_result(self.dispatch.as_ref().unwrap(), "IsAnalyzing", &mut [])
        };
        match result_variant {
            Ok(var) => {
    ...
    pub fn is_physical_model(&self) -> Result<bool, anyErr> {
        let result_variant = unsafe {
            invoke_method_with_result(self.dispatch.as_ref().unwrap(), "IsPhysicalModel", &mut [])
        };
        match result_variant {
            Ok(var) => {
    ...
    pub fn quit(&self) -> Result<(), anyErr> {
        unsafe {
            let result_variant =
                invoke_method_with_result(self.dispatch.as_ref().unwrap(), "Quit", &mut []);
            match result_variant {
                Ok(_) => anyOk(()),
